...
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'V1Staging')
BEGIN
	DROP DATABASE V1Staging;
END

--Create the database.
CREATE DATABASE V1Staging;
GO
...
ALTER DATABASE V1Staging MODIFY FILE 
(NAME = N'V1Staging', SIZE = 100MB , FILEGROWTH = 25MB);
GO

--Change log file initial size and file growth values
ALTER DATABASE V1Staging MODIFY FILE 
(NAME = N'V1Staging_log', SIZE = 100MB , FILEGROWTH = 25MB);
GO
